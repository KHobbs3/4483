# must run "mothur-ALDEx2.R" script first!
# checking which removed SVs are present in other subsets
#comparing control and untreated MS----
aldex.controls <- mothurd[,controls]
aldex.untreated <- mothurd[,untreated]
...
aldex.treated <- mothurd[, c(copaxone, interferon)]
...
mothurd.unfilt <- mothurd[,1:ncol(mothurd)]
...
mothurd.unfilt<-mothurd.unfilt[,c(controls, untreated, copaxone, interferon)] 

aldex.controls.b4 <- mothurd.unfilt[,controls]
aldex.untreated.b4 <- mothurd.unfilt[,untreated]
aldex.treated.b4 <- mothurd.unfilt[,c(copaxone,interferon)]
...
write.table(sv.table, file="svs_b4_and_after_filtering.txt")

tbl <- matrix(c(nrow(common1), nrow(common2), nrow(common3), nrow(common4)))
colnames(tbl) <- "common SVs"
rownames(tbl) <- c("controls/untreated", "untreated/controls")

write.table(tbl, file="common_SVs.txt", sep="\t")
